...
echo "delete from code_mapping;" | db ods
...
echo "COPY code_mapping FROM '/tmp/ip_certification_type_code_mapping.csv' WITH DELIMITER '|';" | db ods
...
echo "COPY code_mapping FROM '/tmp/pb_industry_code_mapping.csv' WITH DELIMITER '|';" | db ods
...
echo "COPY code_mapping FROM '/tmp/pb_region_code_mapping.csv' WITH DELIMITER '|';" | db ods
...
echo "COPY code_mapping FROM '/tmp/code_mapping_customer_sex.csv' WITH DELIMITER '|';" | db ods

echo "select * from code_mapping order by class_id, cd_sno;" | db ods
...
echo "delete from cb_customer_ent;" | db ods
echo "delete from cb_customer_per;" | db ods
...
echo "COPY cb_customer_ent FROM '/tmp/cb_customer_ent.csv' WITH DELIMITER '|';" | db ods
echo "COPY cb_customer_per FROM '/tmp/cb_customer_per.csv' WITH DELIMITER '|';" | db ods


echo "delete from cb_customer_ent where cer_type like '1%';" | db ods
echo "delete from cb_customer_per where cer_type like '2%';" | db ods


echo "update cb_customer_per set sex='nu' where id like 'PER-12%';" | db ods
echo "update cb_customer_per set sex='qt' where id like 'PER-32%';" | db ods

echo "select * from cb_customer_ent order by id;" | db ods
echo "select * from cb_customer_per order by id;" | db ods
